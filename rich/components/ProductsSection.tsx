...
export default function HeroProductsSection() {
  ...
  const [heroProducts, setHeroProducts] = useState<RemoteProduct[]>([]);
    ...
    async function load() {
      try {
        ...
        if (mounted) {
          ...
          setProducts(mapped);
          // Take only first 4 products for hero section
          setHeroProducts(mapped.slice(0, 4));
        }
      } catch (err: any) {
  ...
  return (
    <section className="relative w-full min-h-screen bg-black py-8 lg:py-12 px-4 sm:px-6 lg:px-8 overflow-hidden">
      {/* Enhanced Background Elements */}
      <div className="absolute inset-0 opacity-5">
        <div className="absolute top-0 left-0 w-96 h-96 bg-purple-500 rounded-full blur-3xl animate-pulse" />
        <div className="absolute top-1/2 right-0 w-96 h-96 bg-blue-500 rounded-full blur-3xl animate-pulse" style={{ animationDelay: '2s' }} />
        <div className="absolute bottom-0 left-1/2 w-96 h-96 bg-cyan-500 rounded-full blur-3xl animate-pulse" style={{ animationDelay: '4s' }} />
      </div>

      {/* Animated Grid Pattern */}
      <div className="absolute inset-0 opacity-[0.03] bg-[linear-gradient(rgba(255,255,255,0.1)_1px,transparent_1px),linear-gradient(90deg,rgba(255,255,255,0.1)_1px,transparent_1px)] bg-[size:64px_64px] animate-grid" />
      ...
      <div className="container mx-auto max-w-7xl relative z-10">
        ...
        >
          ...
          <motion.div
            ...
            viewport={{ once: true }}
            className="h-1 bg-gradient-to-r from-blue-500 via-purple-500 to-cyan-500 rounded-full mx-auto mb-6"
          />
          ...
          >
            Discover our handpicked selection of premium products. Each item is carefully curated 
            to bring you the best quality and style.
          </motion.p>
        </motion.div>
        ...
        {loading ? (
          ...
          <div className="text-center py-16">
            ...
            <button
              ...
              className="bg-gradient-to-r from-blue-600 to-purple-600 text-white px-6 py-3 rounded-xl font-semibold hover:shadow-2xl hover:shadow-blue-500/25 transition-all duration-300"
            >
              Try Again
            </button>
          </div>
        ...
        {!loading && !error && products.length > 4 && (
          ...
          >
            <motion.button
              ...
              className="bg-gradient-to-r from-blue-600 to-purple-600 text-white px-8 py-4 rounded-xl font-semibold hover:shadow-2xl hover:shadow-blue-500/25 transition-all duration-300"
            >
              View All Products
            </motion.button>
          </motion.div>
    ...
    >
      ...
      <div className="relative h-72 sm:h-80 w-full overflow-hidden">
        <Image
          ...
          onError={(e) => {
            console.error(`❌ Image failed to load for product: ${product.name}`, e);
      ...
      <div className="p-5 sm:p-6">
        ...
        <div className="text-2xl font-bold text-white mb-4">
          {formatNairaFromSubunit(product.price)}
        </div>
